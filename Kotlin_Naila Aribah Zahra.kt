	...
	fun main() {
	    ...
	    val total = stok.sum()
	    println("Total seluruh barang di gudang: $total")
	}
	...
	fun latihan9() {
	    ...
	    val bookings: List<Booking> = listOf(
	        ...
	        Booking("Alya",  "Kamis",  "08.00–10.00", "Lab AI",     true)
	    )
	
	    val pending: List<Booking> = bookings.filter { !it.disetujui }
	    println("Belum disetujui:")
	    pending.forEach { b: Booking ->
	        println(" - ${b.namaSiswa} | ${b.hari} ${b.jam} @ ${b.lab}")
	    }
	
	    val totalPerLab: Map<String, Int> = bookings.groupingBy { it.lab }.eachCount()
	    println("Total pemesanan per lab:")
	    totalPerLab.forEach { (lab: String, tot: Int) ->
	        println(" - $lab: $tot")
	    }
	
	    val siswaJumat: Set<String> = bookings
	        .filter { it.hari.equals("Jumat", ignoreCase = true) }
	        .map { it.namaSiswa }
	        .toSet()
	    ...
	    val multiBooker: Set<String> = bookings
